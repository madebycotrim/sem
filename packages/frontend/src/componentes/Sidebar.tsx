...
export const Sidebar: FC<SidebarProps> = ({
  ...
  aoMudarSecao,
  aoDeslogar = () => {
    if (window.confirm('Deseja realmente encerrar a sessão do Catraki?')) {
        ...
        <div className="flex flex-col items-center gap-2 pt-2.5 border-t border-slate-100 w-full shrink-0">
          ...
          <div className="relative group">
            <button
              type="button"
              onClick={() => {
                if (window.confirm('Deseja realmente sair do sistema Catraki?')) {
                  aoDeslogar();
                }
              }}
              className="w-10 h-8 rounded-2xl text-slate-400 hover:text-rose-600 hover:bg-rose-50/80 active:bg-rose-100 border border-transparent hover:border-rose-200 flex items-center justify-center transition-all duration-200 cursor-pointer active:scale-95"
            ...
            <div className="absolute left-full ml-3.5 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center z-50 pointer-events-none animate-in fade-in zoom-in-95 duration-150">
              <div className="bg-rose-50 text-rose-700 text-xs font-bold px-3 py-1.5 rounded-2xl shadow-lg border border-rose-200 whitespace-nowrap flex items-center">
