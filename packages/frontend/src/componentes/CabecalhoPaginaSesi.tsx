...
export interface CabecalhoPaginaSesiProps {
  /** Título principal em azul marinho negrito */
  titulo: string;
  /** Subtítulo em caixa alta cinza */
  subtitulo?: string;
  ...
  /** Seletor suspenso (ex: Todas as Escolas / Polos ou Todos os Avisos) */
  seletor?: {
    valor: string;
  ...
  /** Botão de sincronização da Catraki / Fila offline */
  sincronizacao?: {
    aoSincronizar: () => void;
  ...
  /** Callback para botão de exportar relatório / planilha */
  aoExportar?: () => void;
  /** Botão de ação primária (ex: + Novo Paciente, + Novo Atendimento) */
  acaoPrimaria?: {
    rotulo: string;
    aoClicar: () => void;
    icone?: ReactNode;
...
export const CabecalhoPaginaSesi: FC<CabecalhoPaginaSesiProps> = ({
  ...
  return (
    <header
      className={`w-full flex flex-col gap-2.5 mb-3 select-none ${
        fixo
    ...
    >
      ...
      {/* ─── Linha 2: Barra Modular de Filtros e Ações ────────────────────── */}
      {temBarraAcoes && (
          ...
          {busca && (
            <div className="relative flex-1 min-w-[240px]">
              ...
              <input
                ...
                className="w-full pl-9 pr-8 py-1.5 text-xs bg-white border border-slate-200 rounded-lg text-slate-800 placeholder-slate-400 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all shadow-2xs"
              />
              {busca.valor && (
                <button
                  type="button"
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {filtroAvisos && (
              <button
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold rounded-lg border transition-all cursor-pointer shadow-2xs ${
                  filtroAvisos.ativo
            ...
            {/* Dropdown Seletor (Polos / Categorias / Avisos) */}
            {seletor && (
            ...
            {/* Botão Sincronizar Catraki (Verde oficial da referência) */}
            {sincronizacao && (
              <button
                type="button"
            ...
            {acaoPrimaria && (
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 active:bg-blue-800 rounded-lg shadow-xs hover:shadow-sm transition-all active:scale-[0.98] disabled:opacity-50 cursor-pointer"
              >
                {acaoPrimaria.icone || (
