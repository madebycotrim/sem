...
import { validarComprovanteCatraki, type DetalhesValidacaoCatraki, sanitizarCpf } from '../servicos/servicoCatraki.ts';
...
export function ModalValidacaoCatraki({ aberto, paciente, aoFechar }: ModalValidacaoCatrakiProps) {
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-[2px] animate-fade-in font-sans"
      onClick={aoFechar}
    >
      <div
        className="bg-white border border-slate-200/90 rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden flex flex-col animate-modal text-slate-800"
        onClick={(e) => e.stopPropagation()}
      >
        {/* ─── Topo do Modal ─────────────────────────────────────────────── */}
        <div className="flex items-center justify-between px-6 py-4.5 border-b border-slate-100 bg-slate-50/70">
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 rounded-2xl border flex items-center justify-center shadow-2xs ${
              validadoComSucesso
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900 leading-tight">
                {validadoComSucesso ? 'Autenticidade do Termo Catraki' : 'Verificação de Termo no Catraki'}
              </h3>
              <p className="text-[11.5px] text-slate-500 font-medium">
                {validadoComSucesso ? 'Termo de Consentimento Localizado & Homologado' : 'Consulta por CPF na Base Oficial'}
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={aoFechar}
            className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-2xl transition-colors cursor-pointer"
            aria-label="Fechar"
          >
        ...
        <div className="px-6 py-3.5 border-t border-slate-100 bg-slate-50/50 flex justify-end">
          <button
            type="button"
            onClick={aoFechar}
            className="px-5 py-2 text-xs font-bold text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 rounded-2xl transition-colors cursor-pointer shadow-2xs active:scale-[0.98]"
          >
            Fechar
          </button>
        </div>
