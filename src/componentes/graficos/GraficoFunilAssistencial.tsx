...
export const GraficoFunilAssistencial: FC<GraficoFunilAssistencialProps> = ({
  ...
  const baseTotal = etapas[0]?.total || 1;
  const totalConcluido = etapas[etapas.length - 1]?.total || 0;
  const taxaResolutividade = baseTotal > 0 ? ((totalConcluido / baseTotal) * 100).toFixed(1) : '0';
  ...
  return (
    <div className="flex flex-col h-full rounded-2xl border border-slate-200/80 bg-slate-50/30 p-5 sm:p-6 shadow-2xs">
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border-b border-slate-200/70 pb-3.5">
        ...
        <span className="inline-flex items-center gap-1.5 rounded-xl bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-800 border border-emerald-200">
          <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600" />
          {taxaResolutividade}% Eficácia Global
        </span>
      </div>
      ...
      {baseTotal === 0 ? (
        <div className="my-auto flex h-48 items-center justify-center rounded-xl border border-dashed border-slate-200 bg-white text-xs font-medium text-slate-400">
          Sem registros suficientes para projeção do funil assistencial
        </div>
      ) : (
