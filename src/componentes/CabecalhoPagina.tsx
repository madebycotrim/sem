...
export const CabecalhoPagina: FC<CabecalhoPaginaProps> = ({
  ...
  const temBarraAcoes = Boolean(busca || acoesExtras || (busca && temAcoesDireita));

  const renderAcoesDireita = () => (
    <>
  ...
  return (
    ...
    >
      {/* ─── Linha 1: Título e Subtítulo Corporativo + Ações Topo (quando não há busca) ────────────────────────── */}
      <div className="flex items-start sm:items-center justify-between gap-3 w-full">
        ...
        {/* Lado Direito do Título: Quando não há busca, alinha ações aqui via flex justify-between */}
        {!busca && temAcoesDireita && (
      ...
      {/* ─── Linha 2: Barra Contínua de Ações (quando há busca ou ações extras) ───────────────────────────────── */}
      {temBarraAcoes && (
        <div className="flex flex-col sm:flex-row items-stretch sm:items-start justify-between gap-3 w-full">
          ...
          {/* Lado Direito quando há busca */}
          {busca && (temAcoesDireita || acoesExtras) && (
            <div className="flex items-center gap-2.5 flex-wrap sm:flex-nowrap shrink-0 justify-end">
              {renderAcoesDireita()}
              {acoesExtras}
            </div>
          )}

          {/* Ações Extras sem busca (ex: filtros em grade na tela de Relatórios) */}
          {!busca && acoesExtras && (
